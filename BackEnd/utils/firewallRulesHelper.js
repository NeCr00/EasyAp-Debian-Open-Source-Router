...
async function applyFirewallRule(options) {
    ...
    let command = `sudo iptables  -A ${options.chain}`;


    // Check if IP is a range or a single IP
    if (options.ip.includes("-")) {
        if (options.chain === "OUTPUT") {
            command += ` -m iprange --dst-range ${options.ip}`;
        } else {
            command += ` -m iprange --src-range ${options.ip}`;
        }
    } else {
        if (options.chain === "OUTPUT") {
...
async function setCustomFirewallRule(rule, name) { //finished
    ...
    if (stderr) {
        ...
        console.log('cannot add custom firewall rule')
        return false
    }
    else {
